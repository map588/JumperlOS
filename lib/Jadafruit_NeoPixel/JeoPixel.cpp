...
*/
JeoPixel::JeoPixel(uint16_t n, int16_t p, neoPixelType t)
...
/*!
  ...
  @note    This function is deprecated, here only for old projects that
           ...
           type).
*/
void JeoPixel::updateLength(uint16_t n) {
...
/*!
  ...
  @note    This function is deprecated, here only for old projects that
           ...
           (length, pin, type).
*/
  void JeoPixel::updateType(neoPixelType t) {
...
void JeoPixel::show(void) {
  ...
  // NOTE: With async DMA, this is CRITICAL - ensures WS2812 protocol timing
  while (!canShow())
    ;
...
#endif
/*!
  @brief   Force blocking LED update, bypassing DMA completely.
           This method uses direct PIO blocking transfers for guaranteed
           immediate display. Used for critical probe menus where frame
           dropping cannot be tolerated. Slower than show() but reliable.
*/
  void JeoPixel::showBlocking(void) {
  if (!pixels)
    return;

  // Wait for latch time (same as regular show())
  while (!canShow())
    ;

#if defined(ARDUINO_ARCH_RP2040)
  // CRITICAL: Bypass DMA entirely and use reliable blocking PIO method
  // This guarantees the LEDs update before returning, preventing deadlocks
  // in probe menu code that depends on immediate display
  
  if (!pio || (pio_sm < 0)) {
    return; // No PIO available
  }
  
  // Direct blocking PIO transfer - same as DMA fallback path in rp2040Show()
...
*/
void JeoPixel::setPin(int16_t p) {
  if (begun && (pin >= 0))
...
*/
void JeoPixel::setPixelColor(uint16_t n, uint8_t r, uint8_t g,
                                      uint8_t b) {
...
*/
void JeoPixel::setPixelColor(uint16_t n, uint32_t c) {
  if (n < numLEDs) {
...
*/
void JeoPixel::fill(uint32_t c, uint16_t first, uint16_t count) {
  uint16_t i, end;
...
*/
uint32_t JeoPixel::ColorHSV(uint16_t hue, uint8_t sat, uint8_t val) {
...
*/
  uint32_t JeoPixel::getPixelColor(uint16_t n) const {
  if (n >= numLEDs)
...
*/
void JeoPixel::setBrightness(uint8_t b) {
...
*/
uint8_t JeoPixel::getBrightness(void) const { return brightness - 1; }

/*!
  @brief   Fill the whole NeoPixel strip with 0 / black / off.
*/
void JeoPixel::clear(void) { memset(pixels, 0, numBytes); }

// A 32-bit variant of gamma8() that applies the same function
// to all components of a packed RGB or WRGB value.
uint32_t JeoPixel::gamma32(uint32_t x) {
  uint8_t *y = (uint8_t *)&x;
...
*/
void JeoPixel::rainbow(uint16_t first_hue, int8_t reps,
...
*/
  neoPixelType JeoPixel::str2order(const char *v) {
