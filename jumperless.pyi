...
class ProbePad:
    ...
    Supports arithmetic and comparison operations with integers:
    - Comparisons: ==, !=, <, <=, >, >=
    - Arithmetic: +, -, *, //, % (returns int)
    - Conversions: int(pad), float(pad), str(pad)
    - String concatenation: pad + "text" or "text" + pad
    """
    def __bool__(self) -> bool: ...
    def __int__(self) -> int: ...
    def __float__(self) -> float: ...
    ...
    def __add__(self, other) -> Union[int, str]: ...
    def __sub__(self, other) -> int: ...
    def __mul__(self, other) -> int: ...
    def __floordiv__(self, other) -> int: ...
    def __mod__(self, other) -> int: ...
    def __radd__(self, other) -> Union[int, str]: ...
...
class Node:
    ...
    - Comparisons: ==, !=, <, <=, >, >=
    - Arithmetic: +, -, *, //, % (returns int)
    - Conversions: int(node), float(node), str(node)
    """
    def __bool__(self) -> bool: ...
    def __int__(self) -> int: ...
    def __float__(self) -> float: ...
...
def force_service(name: str) -> bool:
    ...
    Returns:
        True if service was found and executed, False otherwise
...
def force_service_by_index(index: int) -> bool:
    """
    Force immediate execution of a service by index (faster than name lookup).
    
    Args:
        index: Service index (get via get_service_index())
        
    Returns:
        True if index valid and service executed, False otherwise
        
    Example:
        >>> btn_idx = get_service_index("ProbeButton")
        >>> force_service_by_index(btn_idx)  # Fastest way
        True
    """
    ...
...
def get_service_index(name: str) -> int:
    """
    Get the index of a service by name for use with force_service_by_index().
    
    Args:
        name: Service name
        
    Returns:
        Service index (0 or higher), or -1 if not found
...
def get_switch_position() -> int:
    """
    Get the current probe switch position.
    
    Returns:
        0 = SWITCH_MEASURE (measure mode)
        1 = SWITCH_SELECT (select mode)
        -1 = SWITCH_UNKNOWN (unknown/uninitialized)
        
    Example:
        >>> if get_switch_position() == SWITCH_MEASURE:
        ...     print("In measure mode")
    """
    ...

def set_switch_position(position: int) -> None:
    """
    Manually set the probe switch position.
    
    Args:
        position: 0 (SWITCH_MEASURE), 1 (SWITCH_SELECT), or -1 (SWITCH_UNKNOWN)
        
    Example:
        >>> set_switch_position(SWITCH_SELECT)
    """
    ...

def check_switch_position() -> int:
    """
    Check the probe switch position using current sensing.
    
    Reads the probe current and updates the switch position based on
    threshold detection (hysteresis prevents oscillation).
    
    Returns:
        0 = SWITCH_MEASURE, 1 = SWITCH_SELECT, -1 = SWITCH_UNKNOWN
...
def clickwheel_get_position() -> int:
    """
    Get the raw clickwheel position counter.
    
    Returns:
        Current position value (can be positive or negative, accumulates as you turn)
...
def clickwheel_reset_position() -> None:
    """
    Reset the clickwheel position counter to 0.
...
def clickwheel_get_direction(consume: bool = True) -> int:
    """
    Get the current clickwheel direction event.
    
    Args:
        consume: If True (default), clears the direction after reading (one-shot).
                 If False, direction persists until consumed.
    
    Returns:
        CLICKWHEEL_NONE (0) - No movement
        CLICKWHEEL_UP (1) - Turning clockwise
        CLICKWHEEL_DOWN (2) - Turning counter-clockwise
        
    The direction state persists until consumed, so you won't miss events
    even if you don't check every loop iteration.
        
    Example (one-shot - default):
        >>> direction = clickwheel_get_direction()  # consume=True
    ...
    Example (persistent):
        ...
        >>> if direction == CLICKWHEEL_UP:
        ...     # Can check multiple times before consuming
...
def clickwheel_get_button() -> int:
    """
    Get the current clickwheel button state.
...
def clickwheel_is_initialized() -> bool:
    """
    Check if the clickwheel hardware is initialized and ready.
    
    Returns:
        True if clickwheel is ready, False otherwise
...
SWITCH_MEASURE: int  # 0 - Measure mode
SWITCH_SELECT: int   # 1 - Select mode
SWITCH_UNKNOWN: int  # -1 - Unknown/uninitialized

# ============================================================================
# Clickwheel (Rotary Encoder) Constants
# ============================================================================

# Direction constants
CLICKWHEEL_NONE: int  # 0 - No movement
CLICKWHEEL_UP: int    # 1 - Turning clockwise
CLICKWHEEL_DOWN: int  # 2 - Turning counter-clockwise

# Button state constants
CLICKWHEEL_IDLE: int           # 0 - Not pressed
CLICKWHEEL_PRESSED: int        # 1 - Just pressed
CLICKWHEEL_HELD: int           # 2 - Being held down
CLICKWHEEL_RELEASED: int       # 3 - Just released
CLICKWHEEL_DOUBLECLICKED: int  # 4 - Double-clicked
