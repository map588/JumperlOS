  ...
  _createMicroPythonModule =
    ...
    async function (moduleArg = {}) {
      ...
      [
        ...
        "_proxy_convert_mp_to_js_then_js_to_mp_obj_jsside",
        "_proxy_convert_mp_to_js_then_js_to_js_then_js_to_mp_obj_jsside",
        "_js_get_proxy_js_ref_info",
        "_has_attr",
        "_lookup_attr",
        "_store_attr",
        "_call0",
      ...
      if (ENVIRONMENT_IS_NODE) {
        ...
        if (
          (numericVersion =
            ...
            1 * numericVersion[2].split("-")[0]) < 16e4
        )
          throw new Error(
        ...
        (scriptDirectory = require("url").fileURLToPath(
          ...
          (readAsync = (filename, binary = !0) => (
            (filename = isFileURI(filename)
              ? new URL(filename)
              : nodePath.normalize(filename)),
            new Promise((resolve, reject) => {
              fs.readFile(filename, binary ? void 0 : "utf8", (err, data) => {
                err ? reject(err) : resolve(binary ? data.buffer : data);
              });
            })
          )),
          !Module.thisProgram &&
      ...
      var wasmBinaryFile,
        ...
        tempI64,
        isDataURI = (filename) =>
          filename.startsWith("data:application/octet-stream;base64,"),
      ...
      function isExportedByForceFilesystem(name) {
        return (
          "FS_createPath" === name ||
          "FS_createDataFile" === name ||
          "FS_createPreloadedFile" === name ||
          "FS_unlink" === name ||
          "addRunDependency" === name ||
          "FS_createLazyFile" === name ||
          "FS_createDevice" === name ||
      ...
      var ptrToString = (ptr) => (
          assert("number" == typeof ptr),
          "0x" + (ptr >>>= 0).toString(16).padStart(8, "0")
        ),
        ...
        warnOnce = (text) => {
          (warnOnce.shown ||= {}),
            warnOnce.shown[text] ||
              ...
              ENVIRONMENT_IS_NODE && (text = "warning: " + text),
              err(text));
        },
        PATH = {
          ...
          basename: (path) => {
            if ("/" === path) return "/";
            var lastSlash = (path = (path = PATH.normalize(path)).replace(
              /\/$/,
              ""
            )).lastIndexOf("/");
          ...
          (randomFill = (() => {
            ...
            if (ENVIRONMENT_IS_NODE)
              try {
                ...
                if (crypto_module.randomFillSync)
      ...
      var wasmTable,
        TTY = {
          ...
          stream_ops: {
            ...
            read(stream, buffer, offset, length, pos) {
              ...
              return (
                bytesRead && (stream.node.timestamp = Date.now()), bytesRead
              );
            },
          ...
          default_tty_ops: {
                ...
                if (!FS_stdin_getChar_buffer.length) {
                  ...
                  if (ENVIRONMENT_IS_NODE) {
                    ...
                    "undefined" != typeof window &&
                      ...
                      null !== (result = window.prompt("Input: ")) &&
                      (result += "\n");
            ...
            ioctl_tcsets: (tty, optional_actions, data) => 0,
            ioctl_tiocgwinsz: (tty) => [24, 80],
          },
          default_tty1_ops: {
            put_char(tty, val) {
              null === val || 10 === val
        ...
        MEMFS = {
          ops_table: null,
          mount: (mount) => MEMFS.createNode(null, "/", 16895, 0),
          ...
          stream_ops: {
            ...
            msync: (stream, buffer, offset, length, mmapFlags) => (
              MEMFS.stream_ops.write(stream, buffer, 0, length, offset, !1), 0
            ),
          },
        },
        ...
        FS = {
          ...
          ErrnoError: class extends Error {
            constructor(errno) {
              for (var key in (super(
                runtimeInitialized
                  ? ((errno) => UTF8ToString(_strerror(errno)))(errno)
                  : ""
              ),
          ...
          create: (path, mode) => (
            (mode = void 0 !== mode ? mode : 438),
            (mode &= 4095),
            (mode |= 32768),
            FS.mknod(path, mode, 0)
          ),
          mkdir: (path, mode) => (
            (mode = void 0 !== mode ? mode : 511),
            (mode &= 1023),
            (mode |= 16384),
            FS.mknod(path, mode, 0)
          ),
          ...
          rename(old_path, new_path) {
            ...
            if (
              ((old_dir = FS.lookupPath(old_path, { parent: !0 }).node),
            ...
            if (
              "." !==
              (relative = PATH_FS.relative(new_path, old_dirname)).charAt(0)
            )
              ...
              ((mode =
                ...
                (flags =
                  "string" == typeof flags
                    ...
                    : flags)
                  ? (4095 & (mode = void 0 === mode ? 438 : mode)) | 32768
                  : 0),
              "object" == typeof path)
            )
              node = path;
          ...
          mmap(stream, length, position, prot, flags) {
            if (2 & prot && !(2 & flags) && 2 != (2097155 & stream.flags))
          ...
          createLazyFile(parent, name, url, canRead, canWrite) {
            ...
            if ("undefined" != typeof XMLHttpRequest) {
              ...
              var properties = { isDevice: !1, contents: new LazyUint8Array() };
        ...
        SYSCALLS = {
          ...
          calculateAt(dirfd, path, allowEmpty) {
            ...
            if (
              ((dir =
                -100 === dirfd
                  ? FS.cwd()
                  : SYSCALLS.getStreamFromFD(dirfd).path),
              0 == path.length)
            ) {
          ...
          getStreamFromFD: (fd) => FS.getStreamChecked(fd),
          varargs: void 0,
          getStr: (ptr) => UTF8ToString(ptr),
        },
      ...
      (FS.createPreloadedFile = (
        ...
        function processData(byteArray) {
          function finish(byteArray) {
            preFinish?.(),
              dontCreateFile ||
                ((parent, name, fileData, canRead, canWrite, canOwn) => {
                  FS.createDataFile(
                    parent,
                    name,
                    fileData,
                ...
                })(parent, name, byteArray, canRead, canWrite, canOwn),
              onload?.(),
        ...
        addRunDependency(dep),
          "string" == typeof url
            ? ((url, onload, onerror, noRunDep) => {
              ...
              })(url, processData, onerror)
            : processData(url);
      }),
        FS.staticInit(),
        void 0 === globalThis.crypto && (globalThis.crypto = require("crypto"));
      var calledRun,
        ...
        wasmImports = {
          ...
          call0_kwarg: function (f_ref, n_kw, key, value, out) {
            ...
            proxy_convert_js_to_mp_obj_jsside(f(a), out);
          },
          call1: function (f_ref, a0, out) {
            const a0_js = proxy_convert_mp_to_js_obj_jsside(a0);
            proxy_convert_js_to_mp_obj_jsside(
              (0, proxy_js_ref[f_ref])(a0_js),
              out
            );
          },
          call1_kwarg: function (f_ref, arg0, n_kw, key, value, out) {
            const f = proxy_js_ref[f_ref],
              a0 = proxy_convert_mp_to_js_obj_jsside(arg0),
              a = {};
            for (let i = 0; i < n_kw; ++i) {
              const k = UTF8ToString(getValue(key + 4 * i, "i32")),
                v = proxy_convert_mp_to_js_obj_jsside(value + 3 * i * 4);
            ...
            proxy_convert_js_to_mp_obj_jsside(f(a0, a), out);
          },
          call2: function (f_ref, a0, a1, out) {
            const a0_js = proxy_convert_mp_to_js_obj_jsside(a0),
              a1_js = proxy_convert_mp_to_js_obj_jsside(a1);
            proxy_convert_js_to_mp_obj_jsside(
              (0, proxy_js_ref[f_ref])(a0_js, a1_js),
              out
            );
          },
          calln: function (f_ref, n_args, value, out) {
            const f = proxy_js_ref[f_ref],
              a = [];
          ...
          emscripten_resize_heap: (requestedSize) => {
            var size,
              alignment,
              oldSize = HEAPU8.length;
            if (
              (assert((requestedSize >>>= 0) > oldSize),
              requestedSize > 2147483648)
            )
              return (
                err(
            ...
            for (var cutDown = 1; cutDown <= 4; cutDown *= 2) {
              ...
              var newSize = Math.min(
                2147483648,
                ((size = Math.max(requestedSize, overGrownHeapSize)),
                assert((alignment = 65536), "alignment argument is required"),
          ...
          fd_seek: function (fd, offset_low, offset_high, whence, newOffset) {
            var lo,
              ...
              offset =
                ((hi = offset_high),
                assert((lo = offset_low) == lo >>> 0 || lo == (0 | lo)),
                assert(hi === (0 | hi)),
          ...
          js_iter_next: function (f_ref, out) {
            ...
            return (
              !ret.done &&
              (proxy_convert_js_to_mp_obj_jsside(ret.value, out), !0)
            );
          },
          ...
          lookup_attr: function (jsref, str, out) {
            ...
            let value = base[attr];
            return (
              (void 0 !== value || attr in base) &&
              ("function" == typeof value &&
                base !== globalThis &&
                ("_ref" in value || (value = value.bind(base))),
          ...
          proxy_convert_mp_to_js_then_js_to_mp_obj_jsside: function (out) {
            proxy_convert_js_to_mp_obj_jsside_force_double_proxy(
              proxy_convert_mp_to_js_obj_jsside(out),
              out
            );
          },
        ...
        wasmExports = (function () {
          ...
          function receiveInstance(instance, module) {
            ...
            return (
              ...
              (cb = wasmExports.__wasm_call_ctors),
        ...
        _fflush =
          (createExportWrapper("__wasm_call_ctors", 0),
          ...
          (Module._mp_js_do_import = createExportWrapper("mp_js_do_import", 2)),
          ...
          (Module._mp_js_repl_init = createExportWrapper("mp_js_repl_init", 0)),
              ...
              (function () {
                if ((checkStackCookie(), Module.postRun))
                  for (
                    ...
                    (cb = Module.postRun.shift()), __ATPOSTRUN__.unshift(cb);
                var cb;
          ...
          (function () {
            if (Module.preRun)
              for (
                ...
                (cb = Module.preRun.shift()), __ATPRERUN__.unshift(cb);
            var cb;
      ...
      if (
        ...
        (Module.setValue = function (ptr, value, type = "i8") {
          switch ((type.endsWith("*") && (type = "*"), type)) {
            case "i1":
            case "i8":
              HEAP8[ptr] = value;
...
function proxy_js_add_obj(js_obj) {
  for (; proxy_js_ref_next < proxy_js_ref.length; ) {
    if (void 0 === proxy_js_ref[proxy_js_ref_next]) {
      ...
      return ++proxy_js_ref_next, (proxy_js_ref[id] = js_obj), id;
    }
    ++proxy_js_ref_next;
  }
  const id = proxy_js_ref.length;
  return (
    (proxy_js_ref[id] = js_obj), (proxy_js_ref_next = proxy_js_ref.length), id
  );
}
